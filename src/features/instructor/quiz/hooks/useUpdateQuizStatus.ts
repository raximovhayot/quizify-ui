...
import { useUpdateQuizStatus as useUpdateQuizStatusBase } from '@/lib/api/hooks/quizzes';
...
export function useUpdateQuizStatus() {
  ...
  const baseUpdateStatus = useUpdateQuizStatusBase();

  return useMutation({
    mutationFn: async (
      data: InstructorQuizUpdateStatusRequest
    ): Promise<void> => {
      await baseUpdateStatus.mutateAsync({ quizId: data.id, status: data.status });
    },
