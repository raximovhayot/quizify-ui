...
export interface QuestionsDisplayHeaderProps {
  count: number;
  showAnswers?: boolean;
...
export function QuestionsDisplayHeader({
  count,
  showAnswers = false,
  onToggleShowAnswers,
  onAddQuestion,
  title,
  subtitle,
}: Readonly<QuestionsDisplayHeaderProps>) {
  const t = useTranslations();
  
  const displayTitle = title || t('common.questions', { fallback: 'Questions' });
  const displaySubtitle = subtitle || t('common.questionsInQuiz', {
    count,
